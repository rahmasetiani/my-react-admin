...
export default function SubjekPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('load /subjek failed:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  const add = async (e) => {
    ...
    // tombol tetap bisa diklik, tapi kalau invalid → tandai merah
    if (!s || isDup(s)) {
    ...
    try {
      ...
      console.error('create /subjek failed:', e2)
    }
  }
  ...
  const editErrSubjek = touchedEdit.subjek && (isEmpty(editSubjek) || isDup(editSubjek, editId))
  ...
  const submitEdit = async (e) => {
    e?.preventDefault?.()
    if (isEmpty(editNama) || isEmpty(editSubjek) || isDup(editSubjek, editId)) {
      setTouchedEdit({ nama:true, subjek:true }) // tandai merah
      return
    }
    try {
      ...
      console.error('update /subjek failed:', e2)
    }
  }
  ...
  const del = async (id) => {
    ...
    try {
      ...
      console.error('delete /subjek failed:', e)
    }
  }
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-80 md:w-96 lg:w-[420px]">
            ...
            />
            <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 select-none text-slate-400">🔎</span>
          </div>
          <button
            onClick={()=>{
              setAddOpen(true)
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1">
            <button onClick={()=>setPage(1)} disabled={curPage===1}
              className="h-9 min-w-9 rounded-lg border border-slate-200 px-2 text-sm disabled:opacity-40" title="Halaman pertama">«</button>
            <button onClick={()=>setPage(p => Math.max(1, p-1))} disabled={curPage===1}
              className="h-9 min-w-9 rounded-lg border border-slate-200 px-2 text-sm disabled:opacity-40" title="Sebelumnya">‹</button>
            ...
            <button onClick={()=>setPage(p => Math.min(totalPages, p+1))} disabled={curPage===totalPages}
              className="h-9 min-w-9 rounded-lg border border-slate-200 px-2 text-sm disabled:opacity-40" title="Berikutnya">›</button>
            <button onClick={()=>setPage(totalPages)} disabled={curPage===totalPages}
              className="h-9 min-w-9 rounded-lg border border-slate-200 px-2 text-sm disabled:opacity-40" title="Halaman terakhir">»</button>
          </div>
        </div>
      </div>
            ...
            <div className="max-h-[80vh] overflow-y-auto p-4 sm:p-5">
              <div className="mb-3 sm:mb-4 flex items-start justify-between gap-3">
                ...
                <button type="button" onClick={()=>setAddOpen(false)}
                  className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 hover:bg-slate-50" aria-label="Close">×</button>
              </div>
              ...
              <input
                ...
                className={`h-10 w-full rounded-xl border px-3 outline-none transition ${
                  addErrSubjek ? 'border-rose-400 ring-1 ring-rose-200' : 'border-slate-200'
                }`}
              />
              {addErrSubjek && (
              ...
              <div className="mt-4 sm:mt-5 flex flex-wrap justify-end gap-2">
                <button type="button" onClick={()=>setAddOpen(false)}
                  className="rounded-xl border border-red-200 bg-red-50 px-3 sm:px-4 py-2 text-red-700 hover:bg-red-100">Batal</button>
                {/* tombol tetap aktif; validasi manual */}
                <button type="submit"
                  className="rounded-xl bg-blue-600 px-3 sm:px-4 py-2 text-white hover:bg-blue-700">
                  Simpan
                </button>
              </div>
            </div>
          ...
          <form onSubmit={submitEdit} className="w-full max-w-sm sm:max-w-md rounded-2xl bg-white shadow-xl">
            <div className="max-h-[80vh] overflow-y-auto p-4 sm:p-5">
              ...
              <input
                ...
                className={`mb-3 h-10 w-full rounded-xl border px-3 outline-none ${
                  editErrNama ? 'border-rose-400 ring-1 ring-rose-200' : 'border-slate-200'
                }`}
              />
              ...
              <input
                ...
                className={`h-10 w-full rounded-xl border px-3 outline-none ${
                  editErrSubjek ? 'border-rose-400 ring-1 ring-rose-200' : 'border-slate-200'
                }`}
              />
              {editErrSubjek && (
                <small className="mt-1 block text-xs text-rose-600">
              ...
              <div className="mt-4 sm:mt-5 flex flex-wrap justify-end gap-2">
                ...
                {/* tombol tetap aktif; validasi manual */}
                <button type="submit"
                  className="rounded-xl bg-blue-600 px-3 sm:px-4 py-2 text-white hover:bg-blue-700">Simpan</button>
              </div>
            </div>
          </form>
        </div>
